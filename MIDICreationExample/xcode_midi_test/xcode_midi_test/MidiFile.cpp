...
//
int MidiFile::writeHeader(const char* filename) {
    fstream output(filename, ios::binary | ios::out);
    ...
    rwstatus = writeHeader(output);
    output.close();
    return rwstatus;
}

int MidiFile::writeHeader(const string& filename) {
    return MidiFile::writeHeader(filename.data());
}

int MidiFile::writeHeader(ostream& out)
{
    int oldTimeState = getTickState();
    if (oldTimeState == TIME_STATE_ABSOLUTE) {
        deltaTicks();
...
//////////////////////////////
//
// MidiFile::writeEndOfTrack --
//
int MidiFile::writeEndOfTrack(const char* filename) {
    ofstream file;
    ...
    rwstatus = writeEndOfTrack(file);
    file.close();
    return rwstatus;
}

int MidiFile::writeEndOfTrack(const string& filename) {
    return MidiFile::writeEndOfTrack(filename.data());
}

int MidiFile::writeEndOfTrack(ostream& out)
{
    char ch;
    ch = 0;
    out << ch;
...
int MidiFile::writeTrack(const char* filename, int aTrack) {
    ofstream file;
    file.open(filename, ios::binary | ios::out | ios::app);
    ...
    rwstatus = writeTrack(file, aTrack);
    file.close();
    return rwstatus;
}


int MidiFile::writeTrack(const string& filename, int aTrack) {
    return MidiFile::writeTrack(filename.data(), aTrack);
}

int MidiFile::writeTrack(ostream& out, int aTrack) {
    int oldTimeState = getTickState();
    if (oldTimeState == TIME_STATE_ABSOLUTE) {
        deltaTicks();
    }
    ...
    // expected data input
    trackdata.clear();
    for (j=0; j<(int)events[aTrack]->size(); j++)
    {
        if ((*events[aTrack])[j].isEndOfTrack())
        {
            // suppress end-of-track meta messages (one will be added
            // automatically after all track data has been written).
            continue;
        }
        writeVLValue((*events[aTrack])[j].tick, trackdata);
        if (((*events[aTrack])[j].getCommandByte() == 0xf0) ||
            ((*events[aTrack])[j].getCommandByte() == 0xf7))
        {
            // 0xf0 == Complete sysex message (0xf0 is part of the raw MIDI).
            // 0xf7 == Raw byte message (0xf7 not part of the raw MIDI).
            // Print the first byte of the message (0xf0 or 0xf7), then
            ...
            // do not insert the VLV byte length yourself, as this code will
            // do it for you automatically.
            trackdata.push_back((*events[aTrack])[j][0]); // 0xf0 or 0xf7;
            writeVLValue((*events[aTrack])[j].size()-1, trackdata);
            for (k=1; k<(int)(*events[aTrack])[j].size(); k++) {
                trackdata.push_back((*events[aTrack])[j][k]);
            }
        }
        else
        {
            // non-sysex type of message, so just output the
            // bytes of the message:
            for (k=0; k<(int)(*events[aTrack])[j].size(); k++) {
                trackdata.push_back((*events[aTrack])[j][k]);
            }
        }
            (*events[aTrack])[j].clear();
    }
...
int MidiFile::addKeySignature(int aTrack, int aTick, int sharps_or_flats, int major_or_minor) {
    MidiEvent* me = new MidiEvent;
    me->makeKeySignature(sharps_or_flats, major_or_minor);
    me->tick = aTick;
    events[aTrack]->push_back_no_copy(me);
    return events[aTrack]->size() - 1;
}
